  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <Link to="/aco/analytics" className="flex items-center gap-1.5 text-xs font-semibold text-brand-600 hover:text-brand-700 bg-brand-50 border border-brand-200 px-3 py-1.5 rounded-lg transition-colors">
        ...
        <ChartCard title="Savings Rate — Quarterly" subtitle="Actual vs target (%)" className="lg:col-span-2" height={250}>
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={savingsData} margin={{ top: 5, right: 10, left: -10, bottom: 0 }}>
              ...
              <Line type="monotone" dataKey="actual" stroke="#6366f1" strokeWidth={2.5} dot={{ r: 4, fill: '#6366f1', strokeWidth: 0 }} name="Actual" />
            </LineChart>
          </ResponsiveContainer>
        </ChartCard>
        ...
        <ChartCard title="Expenditure PMPM" subtitle="Benchmark vs actual YTD ($)" height={220}>
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={expendData} margin={{ top: 5, right: 10, left: -10, bottom: 0 }}>
              ...
              <Area type="monotone" dataKey="actual"    stroke="#6366f1" strokeWidth={2}   fill="url(#acoActualGrad)"                       name="Actual"    />
            </AreaChart>
          </ResponsiveContainer>
        </ChartCard>
        ...
        <ChartCard title="Utilization Trends" subtitle="ED visits/1K · Inpatient/1K · Preventive %" height={220}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={utilData} margin={{ top: 5, right: 10, left: -10, bottom: 0 }}>
              ...
              <Bar dataKey="inpatient"  fill="#6366f1" radius={[3, 3, 0, 0]} name="Inpatient"    barSize={12} />
      ...
      <Card padding={false}>
        <div className="px-5 py-4 border-b border-surface-border flex items-center justify-between">
          <div>
            <h3 className="text-sm font-semibold text-slate-800">Provider Performance</h3>
            <p className="text-xs text-slate-400 mt-0.5">Quality score & cost index — current period</p>
          </div>
          <Link to="/aco/providers" className="text-xs text-brand-600 hover:underline font-medium flex items-center gap-1">
          ...
          <tbody>
            {providers.map(p => (
              <tr key={p.name}>
                ...
                <td className="text-slate-500 text-xs">{p.specialty}</td>
                <td className="text-center">
                  <span className={`font-bold text-sm ${p.quality >= 90 ? 'text-emerald-600' : p.quality >= 80 ? 'text-slate-700' : 'text-amber-600'}`}>
                    {p.quality}
                  </span>
                </td>
                <td className="text-center">
